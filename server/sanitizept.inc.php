...
function fsanitize_email($string)
{
...
function fsanitize_http($string)
{
	if (!preg_match("/^(http:\/\/|https:\/\/)[a-zA-Z0-9_=\&\?\/\-.]+$/", $string)) {
		return FALSE;
	}
...
function fsanitize_ip($string)
{
	if (!preg_match('/\b(?:(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.){3}(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\b/',$string)) 
	{
	 return FALSE;
	}
